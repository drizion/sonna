...
async function main() {
  ...
  const track1 = await parser.parse('https://soundcloud.com/artist-name/track-name');
  console.log(track1);
  ...
  const track2 = await parser.parse('soundcloud.com/artist-name/track-name');
  console.log(track2);
  console.log('');

  // 3. Track público com query params desnecessários
  console.log('3. Track com query params (utm, si):');
  const track3 = await parser.parse('https://soundcloud.com/artist-name/track-name?si=abc123&utm_source=clipboard&utm_medium=text');
  console.log(track3);
  ...
  const track4 = await parser.parse('https://soundcloud.com/artist-name/track-name?in=artist/sets/playlist-name');
  console.log(track4);
  ...
  const track5 = await parser.parse('https://soundcloud.com/artist-name/track-name/s-SecretToken123');
  console.log(track5);
  ...
  const track6 = await parser.parse('https://soundcloud.com/artist-name/track-name/s-SecretToken123?si=abc&utm_source=clipboard');
  console.log(track6);
  ...
  const playlist1 = await parser.parse('https://soundcloud.com/artist-name/sets/playlist-name');
  ...
  const playlist2 = await parser.parse('https://soundcloud.com/drizion/sets/set-foda-atualizado-2026/s-Zny9PbEIV56');
  ...
  const playlist3 = await parser.parse('https://soundcloud.com/drizion/sets/set-foda-atualizado-2026/s-Zny9PbEIV56?si=189ab1eea268454abf941781bbe19d7d&utm_source=clipboard&utm_medium=text&utm_campaign=social_sharing');
  ...
  // URL inválida
  try {
    await parser.parse('https://youtube.com/watch?v=abc123');
  } catch (error: any) {
