import type { Track } from '@music-downloader/shared';
import { useState } from 'react';
import toast from 'react-hot-toast';
import { FiDownload, FiLoader } from 'react-icons/fi';
import { useDownloadTrack, useResolveUrl } from '../hooks/useDownload';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resolveResult.type === 'track') {
        const trackInfo = resolveResult.data as any;
        ...
        toast('Suporte a playlists será implementado em breve!', { icon: '📋' });
      }
    } catch (error: any) {
      ...
      toast.error(error.response?.data?.message || 'Erro ao processar URL');
    }
  };

  const isLoading = resolveUrl.isPending || downloadTrack.isPending;
